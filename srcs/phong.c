...
/*   Updated: 2019/01/23 01:13:16 by kehuang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_clr			phong_shading(t_rtv1 const *core, t_inter const hit,
		t_ray const ray)
{
	t_light	*light_ptr;
	t_vec3	light_dir;
	...
	if (g_state == 1)
	{
		...
		{
			if (not_obstruct(core, hit.obj, hit.pos, light_ptr))
			{
				light_dir = norm_vec3(sub_vec3(light_ptr->pos, hit.pos));
				pxl = add_clr(pxl, diffuse_clr(hit.obj->clr, hit.normal,
							light_dir, 0.70));
				pxl = add_clr(pxl, specular_clr(view, light_dir,
							hit.normal, light_ptr->clr));
			}
...
t_clr			handle_color(t_rtv1 const *core, t_inter const hit,
		t_ray const ray)
{
	t_clr	color_pxl;

	color_pxl = phong_shading(core, hit, ray);
	color_pxl = clamp_clr(color_pxl, 1.0);
	if (g_state == 1)
		if (core->cam.gi != GI_OFF)
			color_pxl = add_clr(color_pxl,
					clamp_clr(glob_illum(core, hit.normal, hit.pos), 1.0));
	return (color_pxl);
}
